...
public class Array {
    ...
    moveElement(customerName, 3, 5);
    moveElement(customerName, 4, 6);
    ...
    public static void moveElement(String[] args, int s1, int s2){
        String temp = args[s1];
        args[s1] = args[s2];
        args[s2] = temp;
    }

    public static void copyElemOver(String[] args, int s1, int s2){
        args[s2] = args[s1];
    }

    public static void removeItemAtIndex(String[] args, String elem){
        for (int i = 0; i < args.length; i++){
            if (args[i] != null && args[i].equals(elem)){
                args[i] = null;

                for (int j = i + 1; j < args.length; j++){
                    moveElement(args, j, j - 1);
                }
                i--;
            }
        }
    }
    public static String[] reverseNames(String[] args){
